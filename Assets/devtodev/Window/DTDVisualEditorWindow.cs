...
namespace DTDEditor {
    public class DTDVisualEditorWindow : EditorWindow {
        ...
        private const string DTDPushBeforeAnalyticsErrorMessage = "Analytics must be enabled before";
        ...
        private void DrawNotificationsBlock() {
            ...
            Rect switchRect = Styles.MakeButton(ViewModel.IsPushMessagesEnabled, () => {
                if (!ViewModel.IsPushMessagesEnabled && !ViewModel.IsAnalyticsEnabled) {
                    EditorUtility.DisplayDialog(DTDTitle, DTDPushBeforeAnalyticsErrorMessage, "Ok");
                } else {
        ...
        private void DrawNotifications() {
            ...
            EditorGUILayout.LabelField("Target", new GUIStyle(Styles.MainTextStyle) { padding = new RectOffset(0, 0, 0, 0) }, GUILayout.Width(120));
            var pushGameObject = EditorGUILayout.ObjectField(ViewModel.PushGameObject, typeof(GameObject), true, GUILayout.ExpandWidth(true), GUILayout.Height(18)) as GameObject;

            if (ViewModel.PushGameObject != pushGameObject) {
                if (ViewModel.IsAnalyticsEnabled) {
                    ViewModel.PushGameObject = pushGameObject;
                } else {
                    EditorUtility.DisplayDialog(DTDTitle, DTDPushBeforeAnalyticsErrorMessage, "Ok");
                }
            }
            ...
            if (ViewModel.PushGameObject != null) {
                EditorGUILayout.BeginHorizontal(new GUIStyle(GUIStyle.none) { padding = new RectOffset(20, 20, 0, 0), fixedHeight = 38 });
                EditorGUILayout.LabelField("Script", new GUIStyle(Styles.MainTextStyle) { padding = new RectOffset(0, 0, -2, 0) }, GUILayout.Width(120));
        ...
        private int DrawPushBlock(int index, string title, string[] methods) {
            EditorGUILayout.BeginHorizontal(new GUIStyle(GUIStyle.none) { padding = new RectOffset(20, 20, 0, 0), fixedHeight = 38 });
            EditorGUILayout.LabelField(title, new GUIStyle(Styles.MainTextStyle) { padding = new RectOffset(0, 0, 0, 0) }, GUILayout.Width(120));
